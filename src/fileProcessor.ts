import { FileInfo, FileStatsConfig, AdaptiveExtractionConfig, FileProcessorConfig, FileStats, Metadata, MediaInfo } from "./types"; // Added specific result types
...
import { AppResult, ok, err, AnyAppError } from "./errors"; // Added AppResult imports
...
export async function processSingleFile(
...
): Promise<AppResult<FileInfo>> { // Update return type
    ...
    const results = await Promise.all([
        ...
        processAdaptiveExtraction(filePath, config.adaptiveExtraction, config.fileStats, cache, workerPool)
    ]);
    ...
    if (fileStatsResult.isErr()) {
        console.error(`Failed to get file stats for ${filePath}:`, fileStatsResult.error);
        return err(fileStatsResult.error); // Propagate the specific error
    }
    if (metadataResult.isErr()) {
         console.error(`Failed to get metadata for ${filePath}:`, metadataResult.error);
        return err(metadataResult.error);
    }
    if (mediaResult.isErr()) {
         console.error(`Failed adaptive extraction for ${filePath}:`, mediaResult.error);
