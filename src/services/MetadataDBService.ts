...
import { FileInfo, FileStats, Metadata, MediaInfo } from '../types'; // Import necessary types
import { AppResult, ok, err, DatabaseError, safeTry } from '../errors';
...
export interface FileInfoRow { // Add export
...
export class MetadataDBService {
    ...
    constructor(dbDirectory: string = '.mediadb', dbFilename: string = 'metadata.sqlite') {
        ...
        try {
            ...
            if (e.code !== 'EEXIST') {
                throw new DatabaseError(`Failed to create database directory: ${e.message}`, { originalError: e });
            }
        }
        ...
        try {
            ...
            console.error(`Failed to open or initialize SQLite database at ${this.dbPath}:`, error);
            throw new DatabaseError(`Failed to initialize SQLite metadata DB: ${error.message}`, { originalError: error });
        }
    }
    ...
    private initSchema(): void {
        ...
        this.db.exec(`CREATE INDEX IF NOT EXISTS idx_files_contentHash ON files (contentHash);`);
        this.db.exec(`CREATE INDEX IF NOT EXISTS idx_files_pHash ON files (pHash);`);
    ...
    private fileInfoToRow(filePath: string, fileInfo: FileInfo): FileInfoRow {
        ...
        return {
            filePath: filePath,
            contentHash: fileInfo.fileStats?.hash ? Buffer.from(fileInfo.fileStats.hash).toString('hex') : null,
    ...
    private rowToFileInfo(row: FileInfoRow): Partial<FileInfo> {
         ...
         const partialFileInfo: Partial<FileInfo> = {
             fileStats: row.contentHash ? {
                 hash: bufferToSharedArrayBuffer(Buffer.from(row.contentHash, 'hex')), // Convert hex -> Buffer -> SharedArrayBuffer
                 size: row.size ?? 0,
             ...
             media: {
                 ...
                 frames: pHashBuffer ? [{ hash: bufferToSharedArrayBuffer(pHashBuffer), timestamp: 0 }] : [], // Convert Buffer -> SharedArrayBuffer
             }
         };
    ...
    public upsertFileInfo(filePath: string, fileInfo: FileInfo): AppResult<void> {
        ...
        return safeTry(() => {
            stmt.run(row);
        }, (e) => new DatabaseError(`Failed to upsert FileInfo for ${filePath}: ${e instanceof Error ? e.message : String(e)}`, { originalError: e instanceof Error ? e : undefined, operation: 'upsert' }));
    }
    ...
    public getFileInfo(filePath: string): AppResult<Partial<FileInfo> | null> {
        ...
        }, (e) => new DatabaseError(`Failed to get FileInfo for ${filePath}: ${e instanceof Error ? e.message : String(e)}`, { originalError: e instanceof Error ? e : undefined, operation: 'get' }));
    }

     /**
     * Retrieves multiple FileInfo entries by file paths.
     */
    public getMultipleFileInfo(filePaths: string[]): AppResult<Map<string, Partial<FileInfo>>> {
        if (filePaths.length === 0) {
            return ok(new Map());
        }
        const placeholders = filePaths.map(() => '?').join(',');
        const stmt = this.db.prepare(`SELECT * FROM files WHERE filePath IN (${placeholders})`);
        ...
        return safeTry(() => {
            ...
            return resultMap;
        }, (e) => new DatabaseError(`Failed to get multiple FileInfo entries: ${e instanceof Error ? e.message : String(e)}`, { originalError: e instanceof Error ? e : undefined, operation: 'getMultiple' }));
    }
    ...
    public findByExactPHash(pHashHex: string): AppResult<FileInfoRow[]> {
        ...
        }, (e) => new DatabaseError(`Failed to find by pHash ${pHashHex}: ${e instanceof Error ? e.message : String(e)}`, { originalError: e instanceof Error ? e : undefined, operation: 'findByPHash' }));
    }
    ...
    public close(): AppResult<void> {
        ...
        }, (e) => new DatabaseError(`Failed to close SQLite DB: ${e instanceof Error ? e.message : String(e)}`, { originalError: e instanceof Error ? e : undefined, operation: 'close' }));
    }
}