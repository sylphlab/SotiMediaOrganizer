...
export class FileSystemError extends AppError {
  constructor(message: string, context?: { path?: string; operation?: string; originalError?: Error }) {
    super(message, context);
  }
}

export class ExternalToolError extends AppError {
  constructor(message: string, context?: { tool?: string; command?: string; exitCode?: number | null; stderr?: string; originalError?: Error }) {
    super(message, context);
  }
}

export class DatabaseError extends AppError {
  constructor(message: string, context?: { operation?: string; key?: string; originalError?: Error }) {
    super(message, context);
  }
}

export class HashingError extends AppError {
  constructor(message: string, context?: { algorithm?: string; filePath?: string; originalError?: Error }) {
    super(message, context);
  }
}

export class ConfigurationError extends AppError {
  constructor(message: string, context?: { setting?: string; value?: unknown }) {
    super(message, context);
  }
}
...
export function safeTry<T>(fn: () => T, errorContext?: string | ((err: unknown) => AnyAppError)): AppResult<T> {
  try {
    return ok(fn());
  } catch (err) {
    ...
    if (typeof errorContext === 'function') {
      ...
      const message = errorContext ? `${errorContext}: ${err instanceof Error ? err.message : String(err)}` : `Operation failed: ${err instanceof Error ? err.message : String(err)}`;
...
export async function safeTryAsync<T>(
  ...
  try {
    ...
    if (typeof errorContext === 'function') {
      ...
      const message = errorContext ? `${errorContext}: ${err instanceof Error ? err.message : String(err)}` : `Async operation failed: ${err instanceof Error ? err.message : String(err)}`;
      // Attempt to create a more specific error if possible, otherwise use base AppError
      if (err instanceof Error && err.message.includes('ENOENT')) { // Example: File not found
         appError = new FileSystemError(message, { originalError: err, operation: 'async operation' });
      } else {
         appError = new AppError(message, { originalError: err });
      }
    }
