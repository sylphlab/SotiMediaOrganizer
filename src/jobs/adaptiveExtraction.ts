import { AdaptiveExtractionConfig, MediaInfo, FileType, FrameInfo, FileStatsConfig } from "../types";
import { LmdbCache } from "../caching/LmdbCache";
import { getFileType, sharedArrayBufferToHex } from "../utils"; // getFileType needs AppResult handling
import { getFileStatsHashKey } from "./fileStats";
import { createSharpInstance, resizeImage, grayscaleImage, imageToBuffer } from "../external/SharpServiceWrapper"; // TODO: Refactor these
import { createFFmpegCommand, probeFile, applyVideoFilters, addOutputOptions } from "../external/FFmpegServiceWrapper"; // TODO: Refactor these
import { WorkerPool } from "../contexts/types";
import { AppResult, ok, err, ExternalToolError, DatabaseError, FileSystemError, UnknownError, AppError, safeTry, safeTryAsync } from "../errors"; // Added AppError
...
async function computePerceptualHashWorker(
...
): Promise<AppResult<SharedArrayBuffer>> { // Update return type
    ...
    try {
        ...
        const buffer = await workerPool.computePerceptualHash(imageBuffer, resolution);
        ...
        return safeTry(() => {
             ...
             return sharedBuffer;
        }, (convError) => new AppError(`Failed to convert worker buffer to SharedArrayBuffer: ${convError instanceof Error ? convError.message : String(convError)}`, { originalError: convError }));
         ...
         return err(new ExternalToolError(
            ...
            { tool: 'workerpool-pHash', originalError: error instanceof Error ? error : undefined }
        )
    );
}
...
): Promise<AppResult<FrameInfo[]>> { // Update return type
    // Wrap the entire promise logic to handle synchronous errors during setup
    return safeTryAsync<FrameInfo[]>(new Promise((resolve, reject) => {
        ...
        const processFrames = () => {
            while (pendingTimestamps.length > 0 && buffer.length >= frameSize) {
                ...
                const framePromise = computePerceptualHashWorker(
                    ...
                    config.resolution,
                    workerPool
                ).then((hashResult): AppResult<FrameInfo> => { // Ensure the .then callback returns AppResult
                    if (hashResult.isErr()) {
        ...
        command
            .on("error", (error) => {
                ...
                reject(new ExternalToolError(`FFmpeg error: ${error.message}`, { tool: 'ffmpeg', originalError: error }));
            })
            .on("end", async () => {
                ...
                try {
                    ...
                    if (firstError && firstError.isErr()) { // Check isErr() again for type safety
            ...
            .on("data", (chunk: Buffer) => {
             ...
             .on('error', (streamError) => { // Add error handling for the stream itself
                 // Reject with a specific error type
                reject(new ExternalToolError(`FFmpeg pipe stream error: ${streamError.message}`, { tool: 'ffmpeg-stream', originalError: streamError }));
            });
    }),
    // Add error handler for safeTryAsync itself
    (error) => new ExternalToolError(`Failed to set up frame extraction promise: ${error instanceof Error ? error.message : String(error)}`, { tool: 'ffmpeg-setup', originalError: error instanceof Error ? error : undefined })
    );
}
...
export async function processAdaptiveExtraction( // Added export keyword
    ...
    cache: LmdbCache,
    workerPool: WorkerPool
): Promise<AppResult<MediaInfo>> { // Update return type
    ...
    if (mediaTypeResult.isErr()) {
        ...
        return err(new FileSystemError(`Could not determine file type for ${filePath}: ${mediaTypeResult.error.message}`, { path: filePath, originalError: mediaTypeResult.error }));
    }
    ...
    const cacheKeyResult = await getFileStatsHashKey(filePath, fileStatsConfig, cache);
     if (cacheKeyResult.isErr()) {
        ...
        return err(new DatabaseError(`Could not get cache key for ${filePath}: ${cacheKeyResult.error.message}`, { key: filePath, originalError: cacheKeyResult.error }));
    }
    ...
    try {
        ...
        if (mediaType === FileType.Image) {
            ...
            if (configCheck.isValid && configCheck.cachedConfig) {
                cacheIsValid = (configCheck.cachedConfig as AdaptiveExtractionConfig)?.resolution === config.resolution;
            }
        } else {
            cacheIsValid = configCheck.isValid;
        }

        if (cacheIsValid) {
            const cacheResult = await cache.getCache<MediaInfo>(JOB_NAME, cacheKey);
            if (cacheResult.hit && cacheResult.data) {
                return ok(cacheResult.data); // Return cached data
            }
        }
    } catch (cacheError) {
         // Log cache check/get error but proceed to calculate
         console.error(`Cache check/get failed for ${filePath} (key: ${cacheKey}), proceeding with calculation: ${cacheError instanceof Error ? cacheError.message : String(cacheError)}`);
         // Do not return error here, allow recalculation
    }
    // --- End Cache Check ---
    ...
    if (mediaType === FileType.Image) {
        ...
        const sharpProcessingPromise = (async (): Promise<Buffer> => { // Ensure IIFE returns Promise<Buffer>
        ...
        const imageProcessingResult = await safeTryAsync<Buffer>(
            ...
            (error) => new ExternalToolError(
                ...
                { tool: 'sharp', originalError: error instanceof Error ? error : undefined }
            )
        );
        ...
        if (imageProcessingResult.isErr()) {
            ...
            const hashResult = await computePerceptualHashWorker(imageProcessingResult.value, config.resolution, workerPool);
            if (hashResult.isErr()) {
                 mediaInfoResult = err(hashResult.error);
            } else {
                 mediaInfoResult = ok({ frames: [{ hash: hashResult.value, timestamp: 0 }], duration: 0 });
            }
        }
        ...
        if (probeResult.isErr()) {
            ...
            if (duration <= 0) {
                 console.warn(`Video ${filePath} has zero or negative duration. Returning empty frames.`);
            ...
            const frameInterval = duration / Math.min(targetFrameCount, config.minFrames);
            ...
            const selectFilter = `select='eq(n,0)+gt(scene,${config.sceneChangeThreshold})*gte(t-prev_selected_t\\,${minInterval})+gte(t-prev_selected_t\\,${frameInterval})'`;
                ...
                const framesResult = await extractFramesWithFilter(filePath, selectFilter, config, workerPool);
                ...
                if (framesResult.isErr()) {
                    ...
                    if (frames.length < 1) {
                        console.warn(`No frames extracted from ${filePath} despite positive duration. Returning empty frames.`);
                        // No error, just empty frames
                        mediaInfoResult = ok({ frames: [], duration });
                    } else {
                         if (frames.length > targetFrameCount && frames.length > config.maxSceneFrames) {
                            // reduceFrames is pure, no AppResult needed unless it could fail
                            frames = reduceFrames(frames, Math.max(targetFrameCount, config.maxSceneFrames));
                        }
                        mediaInfoResult = ok({ frames, duration });
                    }
                }
    ...
    try {
        await cache.setCache(JOB_NAME, cacheKey, mediaInfoResult.value, config);
    } catch (cacheError) {
         // Log cache set error but return the successful result
         console.error(`Cache set failed for ${filePath} (key: ${cacheKey}), but returning calculated result: ${cacheError instanceof Error ? cacheError.message : String(cacheError)}`);
    }
