...
describe("transferFilesFn", () => {
  ...
  beforeEach(async () => { // Make beforeEach async
    ...
    vi.mocked(mockDebugReporter.generateHtmlReports).mockResolvedValue(undefined);
    vi.mocked(mockFileTransferService.transferOrganizedFiles).mockResolvedValue(undefined);
  });
  ...
  it("should transfer files and generate debug report when all paths provided", async () => {
    await transferFilesFn(
      ...
      mockFormat, mockShouldMove, mockDebugReporter, mockFileTransferService, reporterInstance // Pass reporterInstance
    );
    ...
    expect(vi.mocked(fsPromises.mkdir)).toHaveBeenCalledWith(mockDebugDir, { recursive: true });
    expect(vi.mocked(fsPromises.readdir)).toHaveBeenCalledWith(mockDebugDir, { withFileTypes: true });
    expect(vi.mocked(fsPromises.unlink)).not.toHaveBeenCalled();
    // Access mocked methods via top-level mock objects
    expect(vi.mocked(mockDebugReporter.generateHtmlReports)).toHaveBeenCalledWith(mockDupResult.duplicateSets, mockDebugDir);
    ...
    expect(vi.mocked(mockFileTransferService.transferOrganizedFiles)).toHaveBeenCalledWith(
  ...
  it("should skip debug report if debugDir is undefined", async () => {
    ...
    expect(vi.mocked(mockDebugReporter.generateHtmlReports)).not.toHaveBeenCalled();
    expect(vi.mocked(mockFileTransferService.transferOrganizedFiles)).toHaveBeenCalledTimes(1);
  ...
  it("should handle mkdir failure for debugDir", async () => {
   ...
   it("should handle readdir failure for debugDir", async () => {
    ...
    expect(vi.mocked(mockDebugReporter.generateHtmlReports)).toHaveBeenCalledTimes(1);
    expect(vi.mocked(mockFileTransferService.transferOrganizedFiles)).toHaveBeenCalledTimes(1);
  });
   ...
   it("should handle unlink failure during debugDir cleanup", async () => {
    ...
    const fsPromises = await import('fs/promises'); // Import mock inside test
    vi.mocked(fsPromises.readdir).mockResolvedValueOnce([{ name: "old_report.html", isDirectory: () => false, isFile: () => true } as Dirent]);
    ...
    // Check the globally mocked function
    expect(vi.mocked(fsPromises.unlink)).toHaveBeenCalledWith(join(mockDebugDir, "old_report.html"));
  ...
  it("should handle debug report generation failure", async () => {
    ...
    vi.mocked(mockDebugReporter.generateHtmlReports).mockRejectedValue(reportError);
  ...
  it("should handle file transfer failure", async () => {
    ...
    vi.mocked(mockFileTransferService.transferOrganizedFiles).mockRejectedValue(transferError);
  ...
  it("should skip transfer if no files need transferring", async () => {
    ...
    expect(vi.mocked(mockFileTransferService.transferOrganizedFiles)).not.toHaveBeenCalled();
