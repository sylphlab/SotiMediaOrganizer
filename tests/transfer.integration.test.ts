import { jest, describe, it, expect, beforeEach, afterEach } from '@jest/globals'; // Add afterEach
...
class MockCliReporter extends CliReporter {
    ...
    logInfo = jest.fn();
    logSuccess = jest.fn();
    constructor() { super(false); } // Pass verbose=false
}
...
describe('transferFilesFn Integration Tests', () => {
    ...
    it('should call transfer service with correct arguments (copy)', async () => {
        ...
        expect(mockFileTransferService.transferOrganizedFiles).toHaveBeenCalledTimes(1);
        ...
        expect(reporter.stopSpinnerSuccess).toHaveBeenCalledWith(expect.stringContaining("File transfer completed"));
    ...
    it('should handle debug directory creation and report generation', async () => {
        ...
        const readdirSpy = jest.spyOn(fsPromises, 'readdir').mockImplementationOnce(async () => [
             { name: 'old_report.html', isDirectory: () => false, isFile: () => true }, // Mock Dirent object
             { name: 'temp.txt', isDirectory: () => false, isFile: () => true }, // Mock Dirent object
        ] as any);
        ...
        expect(reporter.logWarning).toHaveBeenCalledWith(expect.stringContaining("Debug mode: Duplicate set reports"));
        expect(mockFileTransferService.transferOrganizedFiles).toHaveBeenCalledTimes(1); // Still transfers files
        expect(reporter.stopSpinnerSuccess).toHaveBeenCalled();
     ...
     it('should handle debug directory creation failure', async () => {
         ...
         (mkdirError as any).code = 'EACCES'; // Simulate specific error code if needed

         // Mock fs.mkdir to reject for this test case
         const mkdirSpy = jest.spyOn(fsPromises, 'mkdir').mockImplementationOnce(async () => { throw mkdirError; });
         ...
         expect(mockFileTransferService.transferOrganizedFiles).toHaveBeenCalledTimes(1); // Should still transfer files
     ...
     it('should handle debug report generation failure', async () => {
         ...
         (mockDebugReporter.generateHtmlReports as jest.Mock).mockImplementationOnce(async () => { throw reportError; });
         ...
         expect(mockDebugReporter.generateHtmlReports).toHaveBeenCalledTimes(1);
         expect(reporter.logError).toHaveBeenCalledWith(
             expect.stringContaining(`Failed to generate debug reports in ${debugDir}`),
             reportError
         );
         expect(mockFileTransferService.transferOrganizedFiles).toHaveBeenCalledTimes(1); // Should still transfer files
         expect(reporter.stopSpinnerSuccess).toHaveBeenCalled(); // Transfer should still succeed
    ...
    it('should handle file transfer service failure', async () => {
        ...
        (mockFileTransferService.transferOrganizedFiles as jest.Mock).mockImplementationOnce(async () => { throw transferError; });
    ...
    it('should handle cases with no files to transfer', async () => {
        ...
        expect(mockFileTransferService.transferOrganizedFiles).not.toHaveBeenCalled();
