...
import { FileProcessorConfig, FileInfo, FileType } from '../src/types';
import { AppResult, ok, err, AppError, FileSystemError, DatabaseError } from '../src/errors';
import { bufferToSharedArrayBuffer } from '../src/utils'; // Import buffer utility

import { rmSync, existsSync, mkdirSync } from 'fs';
import { join } from 'path';

import { describe, it, expect, beforeAll, afterAll, beforeEach, afterEach, jest } from '@jest/globals'; // Add Jest imports, including afterEach


// --- Mocking Dependencies ---
// jest.mock('../src/fileProcessor'); // Removed due to potential Bun compatibility issues
...
class MockCliReporter extends CliReporter {
    initializeMultiBar = jest.fn();
    updateProgress = jest.fn();
    stopMultiBar = jest.fn();
    logError = jest.fn();
    logWarning = jest.fn();
    logInfo = jest.fn();
    logSuccess = jest.fn();
    // Add any other methods used by gatherFileInfoFn if necessary
    constructor() { super(false); } // Call super with verbose=false
}
...
const mockExifTool = {
    read: jest.fn(),
    write: jest.fn(),
    end: jest.fn(), // Simple mock
} as unknown as ExifTool;

// Mock WorkerPool (basic mock)
const mockWorkerPool = {
    exec: jest.fn(),
    terminate: jest.fn(), // Simple mock
...
const TEST_GATHERER_CACHE_PATH = join(TEST_GATHERER_DB_DIR, 'gatherer-cache.lmdb');
const TEST_GATHERER_SQLITE_PATH = join(TEST_GATHERER_DB_DIR, 'gatherer-meta.sqlite');
...
describe.skip('gatherFileInfoFn Integration Tests (Skipped in Bun)', () => {
    ...
    afterAll(async () => { // Keep afterAll for cache cleanup
    ...
    beforeEach(async () => { // Renamed from previous beforeEach
        // Reset mocks
        jest.clearAllMocks();
        ...
        dbService = new MetadataDBService(TEST_GATHERER_DB_DIR, 'gatherer-meta.sqlite');
    ...
    it('should process files successfully and store results in DB', async () => {
        ...
        mockProcessSingleFile.mockImplementation(async (filePath) =>
            ok({ ...sampleFileInfo, path: filePath }) // Return unique path in FileInfo
        );
    ...
    it('should handle partial failures during file processing', async () => {
        ...
        mockProcessSingleFile.mockImplementation(async (filePath) => {
            if (filePath === errorFilePath) {
                return err(new FileSystemError('Mock processing error', { path: filePath }));
            }
            return ok({ ...sampleFileInfo, path: filePath });
        });

        const result = await gatherFileInfoFn(files, 1, config, cache, mockExifTool, mockWorkerPool, dbService, reporter);
        ...
        expect(dbCheckOkResult._unsafeUnwrap()?.metadata?.width).toBe(sampleFileInfo.metadata.width);
     ...
     it('should handle DB upsert failures gracefully', async () => {
         ...
         mockProcessSingleFile.mockResolvedValue(ok({ ...sampleFileInfo, path: filePath }));

         // Mock dbService.upsertFileInfo to fail
         const dbError = new DatabaseError('Mock DB upsert failed');
         const upsertSpy = jest.spyOn(dbService, 'upsertFileInfo').mockReturnValue(err(dbError));
         ...
         expect(reporter.logWarning).toHaveBeenCalledWith(
             expect.stringContaining(`DB upsert failed for ${filePath}: ${dbError.message}`)
         );
     ...
     it('should handle empty input map', async () => {
         ...
         const result = await gatherFileInfoFn(files, 2, config, cache, mockExifTool, mockWorkerPool, dbService, reporter);
