...
import { MetadataDBService, FileInfoRow } from '../src/services/MetadataDBService';
...
import {
    ...
    SimilarityConfig,
    DeduplicationResult,
    FileType // Keep FileType if needed elsewhere, otherwise remove
} from '../src/types';
import { LmdbCache } from '../src/caching/LmdbCache';
import { ExifTool } from 'exiftool-vendored';
import { WorkerPool } from '../src/contexts/types';

import { describe, it, expect, beforeAll, afterAll, beforeEach, afterEach, jest } from '@jest/globals'; // Add Jest imports

import { AppResult, ok, err, DatabaseError } from '../src/errors';
...
function generateLshKeysHelper(pHashHex: string | null): (string | null)[] {
    ...
    if (pHashHex && pHashHex.length === 16) { // 64-bit hash
...
async function populateDb(dbService: MetadataDBService, rows: FileInfoRow[]) {
    ...
    for (const row of rows) {
        const pHashBuffer = row.pHash ? bufferToSharedArrayBuffer(Buffer.from(row.pHash, 'hex')) : undefined;
        const fileInfo: Partial<FileInfo> = { // Construct just enough FileInfo
             fileStats: {
        ...
        if (result.isErr()) {
            throw new Error(`Failed to populate DB for ${row.filePath}: ${result.error.message}`);
        }
    }
}
...
describe.skip('deduplicateFilesFn Integration Tests (Skipped in Bun)', () => {
    ...
    beforeAll(() => {
        ...
        const mockCache = { /* basic mock if needed */ } as LmdbCache; // Or use a real one if simple
        const mockFileProcessorConfig = { /* basic mock */ } as FileProcessorConfig;
        const mockExifToolInstance = { end: jest.fn() } as unknown as ExifTool;
        const mockWorkerPoolInstance = { terminate: jest.fn() } as unknown as WorkerPool;
        const mockSimilarityConfig = { /* basic mock */ } as SimilarityConfig;
        // Create a minimal mock ProgramOptions
        programOptions = { // Assign to the variable declared above
             concurrency: 1, move: false, resolution: 64, format: '', windowSize: 5, stepSize: 1, maxChunkSize: 1, minFrames: 1, maxSceneFrames: 1, targetFps: 1, sceneChangeThreshold: 0.1, imageSimilarityThreshold: 0.9, imageVideoSimilarityThreshold: 0.9, videoSimilarityThreshold: 0.9
        };
    ...
    beforeEach(async () => {
        ...
        similarityConfig = { // Define default similarity config
            windowSize: 5,
            stepSize: 1,
        ...
        const mockCacheInstance = { /* mock methods if needed by comparator */ } as LmdbCache;
        const mockExifToolInstance = { end: jest.fn() } as unknown as ExifTool; // Use simple mock
        const mockWorkerPoolInstance = { terminate: jest.fn() } as unknown as WorkerPool; // Use simple mock
        const mockFileProcessorConfig = { /* mock if needed */ } as FileProcessorConfig; // Config passed to deduplicateFilesFn is likely more relevant
    ...
    it('should identify exact duplicates based on pHash', async () => {
        ...
        const result = await deduplicateFilesFn(validFiles, comparator, dbService, similarityConfig, reporter);
        ...
        expect(reporter.startSpinner).toHaveBeenCalledWith("Deduplicating files...");
        expect(reporter.updateSpinnerText).toHaveBeenCalledWith(expect.stringContaining("Finding exact duplicates"));
        expect(reporter.updateSpinnerText).toHaveBeenCalledWith(expect.stringContaining("1 exact duplicate sets"));
     ...
     it('should identify similar files using LSH and similarity check', async () => {
         ...
         const calculateSimilaritySpy = jest.spyOn(comparator, 'calculateSimilarity').mockReturnValue(0.99); // Force high similarity
         ...
         expect(reporter.updateSpinnerText).toHaveBeenCalledWith(expect.stringContaining("Finding similar files using LSH"));
    ...
    it('should treat file as unique if LSH candidates do not meet similarity threshold', async () => {
        ...
        const calculateSimilaritySpy = jest.spyOn(comparator, 'calculateSimilarity').mockReturnValue(0.90); // Below threshold (e.g., 0.98)
     ...
     it('should handle files with no pHash found in DB', async () => {
         ...
         expect(reporter.logWarning).toHaveBeenCalledWith(expect.stringContaining("missing pHash"));
         expect(reporter.stopSpinnerSuccess).toHaveBeenCalled();
     });
     ...
     it('should return error if initial DB query fails', async () => {
         ...
         const getMultipleSpy = jest.spyOn(dbService, 'getMultipleFileInfo').mockImplementation(() => err(dbError)); // Make mock synchronous
         ...
         expect(reporter.stopSpinnerFailure).toHaveBeenCalledWith(expect.stringContaining(dbError.message));
