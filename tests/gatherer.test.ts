...
vi.mock("../src/services/MetadataDBService", () => ({
    MetadataDBService: class {
        upsertFileInfo = vi.fn<(filePath: string, fileInfo: FileInfo) => AppResult<void>>();
        constructor() {}
        close() {}
    },
}));
...
const mockWorkerPool = { exec: vi.fn(), terminate: vi.fn(), stats: vi.fn() } as unknown as WorkerPool;
...
// Mock config
const mockFileStatsConfig: FileStatsConfig = { maxChunkSize: 1024 * 1024 * 100 };
const mockAdaptiveExtractionConfig: AdaptiveExtractionConfig = {
  resolution: 720, sceneChangeThreshold: 0.3, minFrames: 5, maxSceneFrames: 50, targetFps: 1,
};
...
const mockFileInfo: FileInfo = {
  fileStats: { size: 100, createdAt: new Date(), modifiedAt: new Date(), hash: new SharedArrayBuffer(4) },
...
describe("gatherFileInfoFn", () => {
  ...
  beforeEach(async () => { // Make beforeEach async
    ...
    dbServiceMockModule = await vi.importMock("../src/services/MetadataDBService"); // Use renamed variable
    ...
    (fileProcessorMock.processSingleFile as import('vitest').Mock).mockResolvedValue(ok(mockFileInfo));
    (dbInstance.upsertFileInfo as import('vitest').Mock).mockImplementation(() => ok(undefined));
  ...
  it("should process all files successfully and store in DB", async () => {
    ...
    const result = await gatherFileInfoFn(
      files, concurrency, mockConfig, mockCache, mockExifTool, mockWorkerPool, dbInstance, reporterInstance // Use instances
    );
    ...
    expect(dbInstance.upsertFileInfo).toHaveBeenCalledWith("a.jpg", mockFileInfo);
    expect(dbInstance.upsertFileInfo).toHaveBeenCalledWith("b.jpg", mockFileInfo);
    expect(dbInstance.upsertFileInfo).toHaveBeenCalledWith("c.png", mockFileInfo);
  ...
  it("should handle file processing errors", async () => {
    ...
    const concurrency = 1;
    (fileProcessorMock.processSingleFile as import('vitest').Mock).mockImplementation(async (filePath) => { // Use import('vitest').Mock
    ...
    expect(reporterInstance.logError).toHaveBeenCalledWith(expect.stringContaining("Error processing error.jpg"), mockProcError);
    expect(reporterInstance.logWarning).not.toHaveBeenCalled();
   ...
   it("should handle DB upsert errors", async () => {
    ...
    const concurrency = 1;
    (dbInstance.upsertFileInfo as import('vitest').Mock).mockImplementation((filePath: string, fileInfo: FileInfo) => { // Use import('vitest').Mock
    ...
    expect(reporterInstance.logWarning).toHaveBeenCalledWith(expect.stringContaining("DB upsert failed for db_error.jpg")); // Warning logged
  ...
  it("should sort processing by file type and count", async () => {
    ...
    const initArgs = (reporterInstance.initializeMultiBar as import('vitest').Mock).mock.calls[0]; // Use import('vitest').Mock
    // Add check and cast to any[] to bypass TS tuple error
    if (initArgs) {
        const argsArray = initArgs as any[]; // Cast to any[]
